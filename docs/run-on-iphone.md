...
Use this guide to install a personal build from source. After installation, the phone connects directly to the provider selected in Settings and works away from your Mac.
...
- Either an OpenAI project with API billing enabled, or a Google AI Studio API key. Mural uses `gpt-5.6-luna`/`gpt-live-1` for OpenAI and `gemma-4-31b-it`/`gemini-3.8-live` for Google AI Studio.

API usage is paid separately from ChatGPT. Use your own key for this personal build. Never add a key to source code, an Xcode build setting, a screenshot or a GitHub issue.
...
8. Choose your learning and subtitle languages in the welcome screens. In **Settings → Advanced → Use your own API key**, choose OpenAI or Google AI Studio and save that provider’s key. Tap the main conversation button and allow microphone access.
...
| Key rejected or model unavailable | Check the selected provider’s project, billing, permissions and model access. |
